...
exports.store_page_get = asyncHandler(async (req, res, next) => {
  ...
  // Execute store and user queries in parallel
  const [store, user] = await Promise.all([
    Store.findOne({ _id: req.params.id })
      .select('store_name store_logo store_category business_name owner _id')
      .lean()
      .exec(),
    req.session.user ? 
      User.findById(req.session.user._id)
        .select('store_wishlist _id')
        .lean()
        .exec() : 
  ...
  let is_store_owner = user ? req.session.user._id == store.owner.toString() : false;

  console.log("is_store_owner:", is_store_owner);

  // Execute product queries in parallel
  const [all_products, new_products] = await Promise.all([
    Product.find(query)
...
exports.all_products = asyncHandler(async (req, res, next) => {
  const [new_products, products] = await Promise.all([
    Product.find()
      .sort({ date_added: -1 })
      .limit(5)
      .select('name price image store _id')
  ...
  console.log('All products page debug:', {
    store_id: req.params.id,
    products_count: products.length,
    first_product_url: products.length > 0 ? products[0].url : 'No products'
  });
