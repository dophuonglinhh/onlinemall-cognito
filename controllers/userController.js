...
exports.user_detail = asyncHandler(async (req, res, next) => {
  if (!req.session.user) {
    ...
    // Use mongoId for Cognito users, _id for legacy users
    const userId = req.session.user.mongoId || req.session.user._id;
...
exports.user_info_update_get = asyncHandler(async (req, res, next) => {
  const userId = req.session.user.mongoId || req.session.user._id;
  const user = await User.findById(userId);
  const isFirstTime = req.query.first_time === 'true';
  res.render('account_info_update', { 
    user: user, 
    countries: countries, 
    updateBusiness: false,
    isFirstTime: isFirstTime,
    cognitoUser: req.session.user.id ? true : false // Flag to indicate Cognito user
  });
});
...
exports.user_info_update_post = asyncHandler(async (req, res, next) => {
  const { updateProfileCompletion, syncUserSession } = require("../utils/cognitoUserSync");
  const current_user = await User.findOne({ _id: req.params.id });

  const new_user_info = req.body;

  // Update user fields (excluding email for Cognito users)
  if (!current_user.cognito_sub) {
    current_user.mail = new_user_info.mail; // Only update email for non-Cognito users
  }
  ...
  // Only update password for non-Cognito users
  if (!current_user.cognito_sub && new_user_info.password) {
  ...
  await updateProfileCompletion(req.params.id);

  // For Cognito users, rebuild session with updated data
  if (req.session.user.id) {
    ...
    // For legacy users, update session with user data
...
exports.user_brief = asyncHandler(async (req, res, next) => {
  if (req.session.user) {
    ...
    // Get cart information for the navbar (only the count for performance)
    try {
      const userId = req.session.user.mongoId || req.session.user._id;
      
      // Debug logging
      if (!userId) {
        console.error('No userId found in session:', req.session.user);
      ...
      console.error('Error fetching cart data for navbar:', error);
      res.locals.cartItemCount = 0;
    }
  } else {
    res.locals.cartItemCount = 0;
...
exports.user_cart_add_post = asyncHandler( async (req, res, next) => {
  ...
  // Create cart if it doesn't exist (for new Cognito users)
  if (!cart) {
    cart = new Cart({
      _id: new mongoose.Types.ObjectId(),
      user: userId,
...
exports.user_wishlist_get = asyncHandler( async(req, res, next) => {
  const userId = req.session.user.mongoId || req.session.user._id;
  const user = await User.findOne({ _id: userId }).populate('store_wishlist').populate('product_wishlist').exec();
